...
var EventEmitter, Kafka, Promise, Samsa, addPromiseMethods, debug, exit, exitWrapped, exiters, uuid,
  indexOf = [].indexOf;
...
module.exports = Samsa = class Samsa extends EventEmitter {
  constructor(name, methods = {}, config = {}) {
    ...
    this.subscriptions = {};
    this.exitOnClose();
    this.producer.on('ready', (ready) => {
      debug('producer ready', ready);
      return this.emit('producer ready');
    });
    this.producer.on('error', function(error) {
      return debug('producer error', error);
    });
    debug('started');
  }
  ...
  createConsumerGroup() {
    ...
    return this.on('producer ready', async() => {
      await this.createTopics([this.name, this.name + '-response']);
      this.consumer = addPromiseMethods(new Kafka.ConsumerGroup(consumer_group_options, [this.name])); // Subscribes to topic of own name
  ...
  async createTopics(topics) {
    var created;
    created = (await this.producer.createTopicsPromise(topics, true));
    await this.kafka.refreshMetadataPromise(topics);
    return debug('created topics', created);
  }

  async consumeTopic(topic) {
    var added;
    this.consuming_topics.push(topic);
    added = (await this.consumer.addTopicsPromise([topic]));
    return debug('added topic to consumer', added);
  }
  ...
  async request(service, method, ...args) {
    var hook, id, response_topic, sent;
    response_topic = service + '-response';
    if (indexOf.call(this.consuming_topics, response_topic) < 0) {
      await this.consumeTopic(response_topic);
